...
const Cart = {
  ...
  // Sync cart to admin panel for tracking
  syncCartToAdmin() {
    ...
    const cartEntry = {
      ...
      items: cart.map(item => ({
        ...
        image: item.image // Include product image for admin visibility
      })),
    ...
    if (existingIndex !== -1) {
      ...
      // Only add new entry if cart has items
      activeCarts.unshift(cartEntry);
    }

    // Keep only last 100 carts and remove empty ones older than 1 hour
