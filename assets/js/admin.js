...
const AdminApp = {
  ...
  loadDashboard() {
    ...
    const testimonials = AdminData.getTestimonials();
    const inquiries = AdminData.getInquiries();
    const activeCarts = AdminData.getActiveCarts().filter(c => c.itemCount > 0);
  ...
  updateInquiryBadge() {
    const count = AdminData.getUnreadInquiryCount();
  ...
  updateCartBadge() {
    const count = AdminData.getActiveCartCount();
  ...
  openProductModal(productId = null) {
    ...
    this.uploadedImages = []; // Reset uploaded images
    ...
    if (productId) {
      ...
      const product = AdminData.getProduct(productId);
      if (product) {
  ...
  saveProduct(e) {
    ...
    const productData = {
      ...
      images: this.uploadedImages // Use uploaded images array
    };
  ...
  openCategoryModal(categoryId = null) {
    ...
    if (categoryId) {
      ...
      const category = AdminData.getCategory(categoryId);
      if (category) {
  ...
  deleteCategory(id) {
    const productCount = AdminData.getCategoryProductCount(id);
    if (productCount > 0) {
  ...
  openTestimonialModal(testimonialId = null) {
    ...
    if (testimonialId) {
      ...
      const testimonial = AdminData.getTestimonial(testimonialId);
      if (testimonial) {
  ...
  filterInquiries() {
    ...
    let inquiries = AdminData.getInquiries();
  ...
  filterCarts() {
    ...
    let carts = AdminData.getActiveCarts().filter(c => c.itemCount > 0);
  ...
  markCartConverted(sessionId) {
    const cart = AdminData.getActiveCart(sessionId);
    if (cart) {
      // Create an inquiry from the cart
      AdminData.addInquiry({
